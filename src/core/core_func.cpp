...
void blueWhiteMatch(const Mat &src, Mat &match)
{
    ...
    /// segment Blue
    Mat seg_blue;
    hueImgBinarySegment(hue, seg_blue, 110, 30);
    /// segment White
    Mat seg_white;
    whiteBlackSegment(src, seg_white);
    /// Blue dilate
    Mat seg_blue_dilate;
    Mat se_erose = getStructuringElement(MORPH_RECT, Size(5, 5));
    //erode(seg_blue, seg_blue, se_erose);
    Mat se_dilate = getStructuringElement(MORPH_RECT, Size(5, 5));
    dilate(seg_blue, seg_blue_dilate, se_dilate);
    /// White dilate
    Mat seg_white_dilate;
    //erode(seg_white, seg_white, se_erose);
    dilate(seg_white, seg_white_dilate, se_erose);
    /// Blue && White
    Mat blueAndWhite;
